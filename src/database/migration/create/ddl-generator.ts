...
function columnDefinition(column: ColumnSchema, context: DdlContext, options: { autoIncrement?: boolean } = {}): string {
    ...
    if (column.generated) {
        if (context.dialect !== 'mysql') throw new Error('Generated columns are currently supported only for MySQL');
        if (column.autoIncrement) throw new Error(`Generated column ${column.name} cannot be auto-incrementing`);
        if (column.defaultExpression || column.defaultValue !== undefined || column.onUpdateExpression) {
            throw new Error(`Generated column ${column.name} cannot define a default or ON UPDATE expression`);
        }
        parts.push(`GENERATED ALWAYS AS (${column.generated.expression})`, column.generated.storage.toUpperCase());
